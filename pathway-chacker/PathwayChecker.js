document.addEventListener("DOMContentLoaded",function(){const explorerIdInput=document.getElementById("explorer-id-input");const loadingElement=document.getElementById("loading");const errorElement=document.getElementById("error-message");const errorTextElement=document.getElementById("error-text");const dashboardContainer=document.getElementById("dashboard-container");const SCRIPT_URL="https://script.google.com/macros/s/AKfycbygOp9dIBus92vfmgX-ucYXlhRPhLaUcBGLTfZJrZDvK4OtWwAI0jnuBEGTzbpzGukvOw/exec";function getExplorerIdFromHash(){try{return window.location.hash?decodeURIComponent(window.location.hash.substring(1)):null}catch(e){console.error("Error decoding hash:",e);return null}}
