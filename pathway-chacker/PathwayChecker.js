document.addEventListener("DOMContentLoaded",function(){const explorerIdInput=document.getElementById("explorer-id-input");const loadingElement=document.getElementById("loading");const errorElement=document.getElementById("error-message");const errorTextElement=document.getElementById("error-text");const dashboardContainer=document.getElementById("dashboard-container");const SCRIPT_URL="https://script.google.com/macros/s/AKfycbyazhRCyZzt_AqnymBJMFLL0a1nA4FRd32qqTHxHwwKhGrZdOvyqx99RMMc_nU1cZhGbQ/exec";function showLoading(){loadingElement.style.display="block";errorElement.style.display="none";dashboardContainer.style.display="none"}
...
explorerIdInput.addEventListener("keydown",function(event){if(event.key==="Enter"){const explorerId=this.value.trim();if(explorerId){fetchExplorerData(explorerId)}else{showError("Please enter a valid Explorer ID")}}});const urlParams=new URLSearchParams(window.location.search);const explorerId=urlParams.get("id");if(explorerId){explorerIdInput.value=explorerId;fetchExplorerData(explorerId)}})